...
void sobel_SIMD(const uint8_t* img, size_t width, size_t height,
    ...
    __m128i const_p_one = _mm_set1_epi32(+1);
    __m128i const_p_two = _mm_set1_epi32(+2);
    __m128i const_n_one = _mm_set1_epi32(-1);
    __m128i const_n_two = _mm_set1_epi32(-2);

    __m128i zero_mask_first_int = _mm_set_epi32(0xFFFFFFFF,0xFFFFFFFF, 0xFFFFFFFF, 0x00000000);
    __m128i zero_mask_first_two_int = _mm_set_epi32(0xFFFFFFFF,0xFFFFFFFF, 0x00000000, 0x00000000);
    __m128i zero_mask_first_three_int = _mm_set_epi32(0xFFFFFFFF,0x00000000, 0x00000000, 0x00000000);
    ...
    for (size_t gray_y = 0; gray_y < height; gray_y++) {
        for (size_t gray_x = 0; gray_x < width; gray_x += 4) {
            ...
            __m128i bot_left =  _mm_unpacklo_epi16(current_2, _mm_setzero_si128());
            __m128i bot =       _mm_unpacklo_epi16(_mm_srli_si128(current_2, 2), _mm_setzero_si128());
            __m128i bot_right = _mm_unpacklo_epi16(_mm_srli_si128(current_2, 4), _mm_setzero_si128());
            ...
            // Compute gradient magnitude (convert to float for sqrt, then back to int)
            __m128 grad_vert_f = _mm_cvtepi32_ps(gradient_vert);
            __m128 grad_hor_f = _mm_cvtepi32_ps(gradient_hor);
            __m128 gradient_f = _mm_sqrt_ps(_mm_add_ps(_mm_mul_ps(grad_vert_f, grad_vert_f), _mm_mul_ps(grad_hor_f, grad_hor_f)));

            // Clamp to 255 and convert to 8-bit integer
            __m128i gradient_i = _mm_cvtps_epi32(_mm_min_ps(_mm_set1_ps(255.0f), gradient_f));
