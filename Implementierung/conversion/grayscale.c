...
static __inline __m128i convert_rgb_to_gray_5_pixels(__m128i r_3210,
                                                     __m128i g_3210,
                                                     __m128i b_3210,
                                                     const __m128i r_coef_extended,
                                                     const __m128i g_coef_extended,
...
void img_to_grayscale_simd_5_pixels(const uint8_t* img, size_t width, size_t height,
    ...
    // Each coefficient is expanded by 2^15 (not 2^16 as we use signed integers to be able to use _mm_mulhrs_epi16),
    // and rounded to int16 (add 0.5 for rounding). Cannot use shift because of float.
    const __m128i r_coef_extended = _mm_set_epi16(0, 0, 0, r_value_weight_fixed, r_value_weight_fixed, r_value_weight_fixed, r_value_weight_fixed, r_value_weight_fixed);
    const __m128i g_coef_extended = _mm_set_epi16(0,0,0, g_value_weight_fixed, g_value_weight_fixed,g_value_weight_fixed,g_value_weight_fixed,g_value_weight_fixed);
    const __m128i b_coef_extended = _mm_set_epi16(0, 0, 0, b_value_weight_fixed,b_value_weight_fixed,b_value_weight_fixed,b_value_weight_fixed,b_value_weight_fixed);
...
// Calculate 8 Grayscale elements from 8 RGB Values.
static __inline __m128i convert_rgb_to_gray_5_pixels(__m128i r_3210, __m128i g_3210, __m128i b_3210,
                                                     const __m128i r_coef_extended,
                                                     const __m128i g_coef_extended,
                                                     const __m128i b_coef_extended) {
    ...
    // Multiply input elements by 64 for improved accuracy.
    r_3210 = _mm_slli_epi16(r_3210, 6);
    g_3210 = _mm_slli_epi16(g_3210, 6);
    b_3210 = _mm_slli_epi16(b_3210, 6);

    // Use the special intrinsic _mm_mulhrs_epi16 that calculates round((r * r_coef) / 2^15).
    // Calculate gray_value using fixed point multiplication and adding up the results.
    __m128i gray_76543210 = _mm_add_epi16(_mm_add_epi16( _mm_mulhrs_epi16(r_3210, r_coef_extended),
                                                         _mm_mulhrs_epi16(g_3210, g_coef_extended)),
                                          _mm_mulhrs_epi16(b_3210, b_coef_extended));
...
static __inline void extract_r_g_b_sorted_another(const __m128i r_5_bgr_4_bgr_3_bgr_2_bgr_1_bgr_0,
                                                   const __m128i bgr_7_bgr_6_bgr_5_bgr_4,
                                                   __m128i* r_43210,
                                                   __m128i* g_43210,
                                                   __m128i* b_43210);

static __inline __m128i convert_rgb_to_gray_another(__m128i r_3210,
                                                     __m128i g_3210,
                                                     __m128i b_3210,
                                                     const __m128i r_coef_extended,
                                                     const __m128i g_coef_extended,
...
void img_to_grayscale_simd_another(const uint8_t* img, size_t width, size_t height,
    ...
    for (; index_rgb < simd_size; index_rgb += 24, index_gray += 8) {
        ...
        // Separate RGB, and put together Red Values, Green Values and Blue Values (together in same XMM register).
        // Result is also unpacked from uint8 to uint16 elements.
        extract_r_g_b_sorted_another(r_5_bgr_4_bgr_3_bgr_2_bgr_1_bgr_0, bgr_7_bgr_6_bgr_5_bgr_4,
                                      &r_43210, &g_43210, &b_43210);
        ...
        if (a > 0.999) {
            ...
            // Calculate 8 Y elements.
            __m128i gray_16_76543210 = convert_rgb_to_gray_another(r_43210, g_43210, b_43210,
                                                                    r_coef_extended,
                                                                    g_coef_extended,
                                                                    b_coef_extended);
...
static __inline void extract_r_g_b_sorted_another(const __m128i r_5_bgr_4_bgr_3_bgr_2_bgr_1_bgr_0,
    ...
    *r_43210 = _mm_unpacklo_epi8(r_3210_b_3210_g_3210_r_3210, _mm_setzero_si128());
    *r_43210 = _mm_unpacklo_epi64(*r_43210, _mm_unpacklo_epi8(r_7654_b_7654_g_7654_r_7654, _mm_setzero_si128()));


    __m128i unclean_colour_3210 = _mm_srli_si128(r_3210_b_3210_g_3210_r_3210, 4);
    __m128i unclean_colour_7654 = _mm_srli_si128(r_7654_b_7654_g_7654_r_7654, 4);
    *g_43210 = _mm_unpacklo_epi8(unclean_colour_3210, _mm_setzero_si128());
    *g_43210 = _mm_unpacklo_epi64(*g_43210, _mm_unpacklo_epi8(unclean_colour_7654, _mm_setzero_si128()));

    // BLUE: Put 8 Blue Values in lower part.
    unclean_colour_3210 = _mm_srli_si128(unclean_colour_3210, 4);
    unclean_colour_7654 = _mm_srli_si128(unclean_colour_7654, 4);
    *b_43210 = _mm_unpacklo_epi8(unclean_colour_3210, _mm_setzero_si128());
    *b_43210 = _mm_unpacklo_epi64(*b_43210, _mm_unpacklo_epi8(unclean_colour_7654, _mm_setzero_si128()));
}

// Calculate 8 Grayscale elements from 8 RGB Values.
static __inline __m128i convert_rgb_to_gray_another(__m128i r_3210, __m128i g_3210, __m128i b_3210,
                                                     const __m128i r_coef_extended,
                                                     const __m128i g_coef_extended,
                                                     const __m128i b_coef_extended) {
