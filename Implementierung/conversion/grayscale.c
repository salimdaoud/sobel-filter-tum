...
static __inline __m128i convert_rgb_to_gray_5_pixels(__m128i r_3210, __m128i g_3210, __m128i b_3210,
    ...
    // Use the special intrinsic _mm_mulhrs_epi16 that calculates round((r * r_coef) / 2^15).
    // Calculate gray_value using fixed point multiplication and adding up the results.
    __m128i gray_76543210 = _mm_add_epi64(_mm_add_epi64( _mm_mulhrs_epi16(r_3210, r_coef_extended),
...
void img_to_grayscale_simd_goated(const uint8_t* img, size_t width, size_t height,
                                    float a, float b, float c, uint8_t* gray) {
    size_t rgb_size = height * width * 3;
    size_t index_rgb = 0;
    size_t index_gray = 0;

    __m128 rgbr_coefficients = _mm_set_ps(a, c, b, a);
    __m128 gbrg_coefficients = _mm_set_ps(b, a, c, b);
    __m128 brgb_coefficients = _mm_set_ps(c, b, a, c);

    //Process one row per iteration.
    for (; index_rgb < rgb_size; index_rgb += 6, index_gray += 2) {

        // Load 8 elements of each color channel R,G,B from first row. Load 24 (8 * 3 values) unaligned
        // variable name is displayed as 128 bit integer. Within memory, values displayed in reversed order due to
        // Little-Endianness
        __m128 rgbr_values = _mm_cvtepi32_ps(_mm_cvtepu8_epi32(_mm_loadu_si64((__m128i*) (img + index_rgb))));
        __m128 gbrg_values = _mm_cvtepi32_ps(_mm_cvtepu8_epi32(_mm_loadu_si64((__m128i*) (img + index_rgb + 1))));
        __m128 brgb_values = _mm_cvtepi32_ps(_mm_cvtepu8_epi32(_mm_loadu_si64((__m128i*) (img + index_rgb + 2))));

        __m128 rgbr_values_weighted = _mm_mul_ps(rgbr_values, rgbr_coefficients);
        __m128 gbrg_values_weighted = _mm_mul_ps(gbrg_values, gbrg_coefficients);
        __m128 brgb_values_weighted = _mm_mul_ps(brgb_values, brgb_coefficients);

        __m128i intermediate_result_byte = _mm_cvttps_epi32(_mm_add_ps(_mm_add_ps(rgbr_values_weighted,gbrg_values_weighted), brgb_values_weighted));

        gray[index_gray] = (uint8_t)_mm_cvtsi128_si32(intermediate_result_byte);
        gray[index_gray + 1] = (uint8_t)_mm_cvtsi128_si32(_mm_srli_si128(intermediate_result_byte, 12));
    }
    // Handle the rest of the pixels. Naive implementation as there are at most 23 values.
    size_t pixels_left = rgb_size % 12;
