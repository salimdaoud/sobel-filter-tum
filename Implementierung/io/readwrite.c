...
void read_ppm_file(const char* file_name, int* width, int* height, uint8_t** pixel_rgb_data, bool use_io_threading) {
    ...
    if (max_val < 0 || max_val > 255) {
        fprintf (stderr, "Failed! Provided file : %s is not a regular PPM image. Non valid max_val.", file_name);
        goto cleanup;
    }

   size_t rgb_values = *width * *height * 3;
   if (file_size !=  rgb_values + header_size) {
        fprintf(stderr, "Failed! Provided file %s is not a regular PPM image. Number of pixels is ambiguous.", file_name);
    }
